...
2. 双击 **`scripts\创建快捷方式.bat`**。
   它会在**桌面**和**开始菜单**创建快捷方式「DSH 快速启动」，并把结果显示在窗口里。
3. 之后双击桌面上的「DSH 快速启动」即可打开 DSH。

如果第 2 步失败，同目录会生成 `Install-Log.txt`，里面有明确的失败原因。
若提示位置不可写，右键该 bat 选 **以管理员身份运行**再跑一次。

> 这个 bat 的**文件名是中文，但内容全部是 ASCII**。这不是随意约定：
> 中文控制台代码页是 936（GBK），若 `.bat` 内容里出现 UTF-8 中文，
> cmd.exe 会读坏引号配对，脚本在第一行就静默中止。详见下方"踩过的坑"。
...
- 换图**不会**让本仓库变成纯 MIT：已分发的图标文件仍受 CC BY-NC-SA 4.0 约束，
  详见 [ICON-CREDITS.md](ICON-CREDITS.md)

### 换了图标，桌面却没变？

Windows 的**图标缓存按「图标文件路径」做键**。如果只替换 `dsh-whale.ico` 的内容
而路径没变，资源管理器很可能继续显示旧图标。

最省事的办法：**把新图标换成一个新的文件名**，再让快捷方式指向它。
另外，注销一次再登录（或重启）也一定能刷新。

> 早期版本里有一个 `Fix-Desktop-Icon.bat` 专门处理这件事（它会删除
> `iconcache_*.db` 并重启资源管理器）。因为它属于"系统级"操作、风险偏高，
> 已从本仓库移除。需要时手动执行即可：
>
> ```bat
...
└─ scripts/
   ...
   ├─ Create-DSH-Shortcut.ps1 创建 .lnk（决定用哪个图标）
   ├─ launch.bat             启动器（主入口）
...
2. **`.ps1` 必须存成 UTF-8 with BOM。**
   ...
   **甚至导致语法错误、脚本完全无法运行**（本项目就踩过：补 BOM 前的
   `Install-Shortcut.ps1` 直接抛 "字符串缺少终止符"）。

3. **图标缓存按路径做键。**
   换图标要换文件名，不能只换内容（见上文"换了图标，桌面却没变？"）。

4. **不要用单一的 `Test-Path` 判断文件是否创建成功。**
   在沙箱、按需加载的虚拟磁盘、杀软实时扫描等环境下，文件刚写完的瞬间
   `Test-Path` 可能返回 false，于是**明明建成功却被报成失败**（本项目实测踩到）。
   现在改为「重试 + 多重证据」：文件长度 > 0 且能用 Shell 打开并读到 TargetPath。

5. **"目标位置已经有文件"≠"这次安装成功了"。**
   如果覆盖失败但旧文件还在，必须如实报告"可能不是最新版本"，
   否则用户会以为已经更新，实际上还是旧的。

6. **不能把"已存在的快捷方式"直接当成可复用的模板。**
   一个 `.lnk` 可能指向**已经被删除的目录**，此时它"文件存在"但完全无效：
   桌面图标会变成**白色方块**，双击报**找不到文件**。判断可用性必须同时看
   目标脚本、参数里引用的脚本、以及图标文件是否都还在。

7. **先本地生成、再复制出去。**
   早期版本先写桌面，失败才退到本目录，而那个退路被状态变量挡住，
   于是桌面写不进去时本地也没生成，却照样打印"完成" —— 调用方永远拿不到
   可用的源快捷方式。现在固定为"先在脚本目录生成权威 .lnk，再复制到桌面/开始菜单"。

8. **移动/重命名过项目目录后，必须重装快捷方式。**（见下条）
...
**本仓库不是纯 MIT**：本项目自己编写的**全部脚本代码**采用 **MIT**
...
并可能清理 Windows 图标缓存、重启资源管理器——**运行前请先读脚本注释**。
...
"DSH 快速启动" shortcut on your Desktop and in the Start Menu. After that,
double-click that shortcut to start DSH.
...
- `.bat` / `.vbs` files must stay **pure ASCII**. On a Chinese Windows console
  (code page 936/GBK), UTF-8 Chinese text inside a batch file corrupts quote
  pairing and aborts the script on line 1 with no error output.
- `.ps1` files must be saved as **UTF-8 with BOM**, or PowerShell 5.1 mis-decodes them.
...
**License** — this repository is **not MIT-only**. Our own scripts and the
code-drawn icon are MIT (author: subtob). The bundled whale-girl artwork and any
`.ico` derived from it are **CC BY-NC-SA 4.0** and are excluded from the MIT
grant: attribution required, non-commercial only, share-alike. See
[LICENSE](LICENSE) and [ICON-CREDITS.md](ICON-CREDITS.md).

**AI generation notice** — built with the assistance of DeepSeek Harness
(`dsh`) running model **DSH V4.1 FLASH** (`deepseek-flash`), under the direction
and review of the human author subtob. See [NOTICE.md](NOTICE.md).
